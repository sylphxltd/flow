export const CoordinationHooksPlugin = async ({ project, client, $, directory, worktree }) => {
    return {
        /**
...
async function getRecentEventsForAgent(agentId, maxAge = 300000) {
...
function shouldBroadcastEvent(response) {
    ...
    return responseText && (responseText.includes('✅') ||
...
async function broadcastAgentEvent(agentId, message) {
    try {
        const eventData = {
            message: message,
            timestamp: Date.now()
        ...
        const responseText = typeof message === 'string' ? message :
            message?.content ||
                message?.text ||
