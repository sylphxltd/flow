...
import { createAIStream, getSystemStatus, buildSystemStatusFromMetadata, injectSystemStatusToOutput, } from '@sylphx/code-core';
...
export function streamAIResponse(opts) {
    return observable((observer) => {
        ...
        (async () => {
            try {
                const { sessionRepository, aiConfig, sessionId: inputSessionId, provider: inputProvider, model: inputModel, userMessage, attachments = [], abortSignal, } = opts;
                ...
                if (!sessionId) {
                    ...
                    const newSession = await sessionRepository.createSession(inputProvider, inputModel);
                ...
                const messageId = await sessionRepository.addMessage(sessionId, 'user', [{ type: 'text', content: userMessage }], attachments, undefined, undefined, {
