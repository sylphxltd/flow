...
export const messageRouter = router({
    ...
    getBySession: publicProcedure
        ...
        return await ctx.sessionRepository.getMessagesBySession(input.sessionId, input.limit, input.cursor);
    }),
    /**
    ...
    add: moderateProcedure
        ...
        if (!sessionId) {
            ...
            const session = await ctx.sessionRepository.createSession(input.provider, input.model, input.agentId || 'coder');
        ...
        const messageId = await ctx.sessionRepository.addMessage(sessionId, input.role, input.content, input.attachments, input.usage, input.finishReason, input.metadata, input.todoSnapshot, input.status);
    ...
    updateStatus: moderateProcedure
        ...
        .mutation(async ({ ctx, input }) => {
        await ctx.sessionRepository.updateMessageStatus(input.messageId, input.status, input.finishReason);
        eventBus.emitEvent({
            type: 'message-updated',
            messageId: input.messageId,
            field: 'status',
