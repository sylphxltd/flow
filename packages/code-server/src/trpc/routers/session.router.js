...
export const sessionRouter = router({
    ...
    create: strictProcedure
        ...
        const session = await ctx.sessionRepository.createSession(input.provider, input.model, input.agentId || 'coder', input.enabledRuleIds || [] // Initialize with provided rules or empty
        );
    ...
    updateProvider: moderateProcedure
        ...
        .mutation(async ({ ctx, input }) => {
        await ctx.sessionRepository.updateSessionProvider(input.sessionId, input.provider, input.model);
        eventBus.emitEvent({
            type: 'session-updated',
            sessionId: input.sessionId,
            field: 'provider',
