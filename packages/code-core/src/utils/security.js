...
export const securitySchemas = {
    ...
    branchName: z
        ...
        .regex(/^[a-zA-Z0-9/_-]+$/, 'Branch name can only contain letters, numbers, slashes, hyphens, and underscores')
    ...
    filePath: z
        ...
        .refine((filePath) => !filePath.includes('..'), 'File path cannot contain ".." for path traversal protection')
    ...
    commandArg: z
        ...
        .refine((arg) => !/[<>|;&$`'"\\]/.test(arg), 'Command argument contains potentially dangerous characters'),
    ...
    url: z
        ...
        .refine((url) => url.startsWith('https://') || url.startsWith('http://localhost'), 'URL must be HTTPS or localhost')
...
export const commandSecurity = {
     ...
     */
    async safeExecFile(command, args, options = {}) {
        // Validate command
...
export const securityMiddleware = {
     ...
     */
    rateLimit: (limiter, getIdentifier) => {
        return (req, res, next) => {
