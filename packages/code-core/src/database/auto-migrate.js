 ...
 */
async function needsFileMigration(db) {
    try {
 ...
 */
async function cleanupOldJSONFiles() {
    try {
        const files = await readdir(SESSION_DIR);
        const sessionFiles = files.filter((f) => f.endsWith('.json') && !f.startsWith('.'));
        if (sessionFiles.length === 0) {
 ...
 */
async function runSchemaMigrations(db) {
 ...
 */
async function migrateSessionFiles(db, onProgress) {
    const repository = new SessionRepository(db);
        ...
        try {
            ...
            for (const message of session.messages) {
                ...
                if (message.attachments && Array.isArray(message.attachments) && message.attachments.length > 0) {
                    const validAttachments = message.attachments.filter((a) => a && typeof a === 'object' && a.path && a.relativePath);
                    if (validAttachments.length > 0) {
                        normalizedAttachments = validAttachments;
                    }
                }
                await repository.addMessage(session.id, message.role, message.content, normalizedAttachments, message.usage, message.finishReason, message.metadata, message.todoSnapshot);
            }
            // Update todos
    ...
    await fsReadFile(MIGRATION_FLAG, 'utf8').catch(() => require('fs').writeFileSync(MIGRATION_FLAG, new Date().toISOString()));
 ...
 */
export async function autoMigrate(onProgress) {
