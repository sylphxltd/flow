 ...
 */
async function searchFiles(dir, pattern, maxResults, results = []) {
...
export const grepTool = tool({
    ...
    execute: async ({ pattern, path, output_mode = 'files_with_matches', type, glob: globPattern, '-i': caseInsensitive = false, '-n': showLineNumbers = false, '-A': afterContext = 0, '-B': beforeContext = 0, '-C': aroundContext, multiline = false, head_limit, }) => {
        const searchDir = path || process.cwd();
        ...
        if (caseInsensitive)
            regexFlags += 'i';
        if (multiline)
            regexFlags += 's'; // 's' flag makes . match newlines
