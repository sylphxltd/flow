...
export function useKeyboardNavigation(props) {
    const { input, cursor, isStreaming, pendingInput, pendingCommand, filteredFileInfo, filteredCommands, multiSelectionPage, multiSelectionAnswers, multiSelectChoices, selectionFilter, isFilterMode, freeTextInput, isFreeTextMode, selectedCommandIndex, selectedFileIndex, skipNextSubmit, lastEscapeTime, inputResolver, commandSessionRef, abortControllerRef, cachedOptions, setInput, setCursor, setShowEscHint, setMultiSelectionPage, setSelectedCommandIndex, setMultiSelectionAnswers, setMultiSelectChoices, setSelectionFilter, setIsFilterMode, setFreeTextInput, setIsFreeTextMode, setSelectedFileIndex, setPendingInput, setPendingCommand, addLog, addMessage, addAttachment, setAttachmentTokenCount, createCommandContext, getAIConfig, currentSessionId, currentSession, } = props;
    useInput(async (char, key) => {
        ...
        if (pendingInput && inputResolver.current) {
            ...
            if (pendingInput.type === 'selection') {
                ...
                const filteredOptions = currentQuestion.options.filter((option) => option.label.toLowerCase().includes(selectionFilter.toLowerCase()) ||
                ...
                if (isFreeTextMode) {
                    ...
                    if (key.return) {
                        ...
                        const resultSessionId = await addMessage(sessionIdToUse, 'user', customValue, undefined, undefined, undefined, undefined, undefined, provider, model);
                        ...
                        if (isSingleQuestion) {
                            ...
                            if (allAnswered) {
                                ...
                                const nextUnanswered = questions.findIndex((q, idx) => idx > multiSelectionPage && !newAnswers[q.id]);
                                if (nextUnanswered !== -1) {
                ...
                if (key.return) {
                    ...
                    if (currentQuestion.multiSelect) {
                        ...
                        const model = aiConfig?.defaultModel || 'anthropic/claude-3.5-sonnet';
                        const sessionIdToUse = commandSessionRef.current || currentSessionId;
                        const resultSessionId = await addMessage(sessionIdToUse, 'user', choicesArray.join(', '), undefined, undefined, undefined, undefined, undefined, provider, model);
                        if (!commandSessionRef.current) {
                            commandSessionRef.current = resultSessionId;
                        }
                        if (isSingleQuestion) {
                        ...
                        if (selectedOption) {
                            ...
                            const model = aiConfig?.defaultModel || 'anthropic/claude-3.5-sonnet';
                            const sessionIdToUse = commandSessionRef.current || currentSessionId;
                            const resultSessionId = await addMessage(sessionIdToUse, 'user', selectedOption.label, undefined, undefined, undefined, undefined, undefined, provider, model);
                            if (!commandSessionRef.current) {
                                commandSessionRef.current = resultSessionId;
                            }
                            if (isSingleQuestion) {
        ...
        if (filteredFileInfo.hasAt && filteredFileInfo.files.length > 0 && !pendingInput) {
            ...
            if (key.downArrow) {
                setSelectedFileIndex((prev) => prev < filteredFileInfo.files.length - 1 ? prev + 1 : prev);
                return;
            }
        ...
        // Handle command autocomplete navigation
            ...
            if (key.return) {
                ...
                if (selected) {
                    ...
                    const model = aiConfig?.defaultModel || 'anthropic/claude-3.5-sonnet';
                    const sessionIdToUse = commandSessionRef.current || currentSessionId;
                    const resultSessionId = await addMessage(sessionIdToUse, 'user', selected.label, undefined, undefined, undefined, undefined, undefined, provider, model);
                    if (!commandSessionRef.current) {
                        commandSessionRef.current = resultSessionId;
                    }
                    ...
                    if (response) {
                        await addMessage(commandSessionRef.current, 'assistant', response, undefined, undefined, undefined, undefined, undefined, provider, model);
                    }
                }
                return;
            }
