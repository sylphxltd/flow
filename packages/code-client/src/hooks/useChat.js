...
import { createAIStream, getSystemStatus, buildSystemStatusFromMetadata, injectSystemStatusToOutput, buildTodoContext, } from '@sylphx/code-core';
...
class FileContentCache {
    cache = new Map();
    accessOrder = new Set(); // Track LRU order
...
export function useChat() {
    ...
    const sendMessage = async (message, options) => {
        ...
        const { attachments, abortSignal, onComplete, onAbort, onError, onFinish, onToolCall, onToolResult, onToolInputStart, onToolInputDelta, onToolInputEnd, onToolError, onReasoningStart, onReasoningDelta, onReasoningEnd, onTextStart, onTextDelta, onTextEnd, onUserInputRequest, } = options;
        ...
        try {
            ...
            addMessage(currentSessionId, 'user', [{ type: 'text', text: message }], // FIX: Use 'text' field
            attachments, undefined, // usage (only for assistant messages)
            ...
            if (updatedSession.messages.length === 1 && !updatedSession.title) {
                ...
                if (autoGenerateTitle) {
                    ...
                    generateSessionTitleWithStreaming(message, provider, modelName, providerConfig, (chunk) => {
                ...
                if (msg.role === 'user') {
                    ...
                    if (msg.attachments && msg.attachments.length > 0) {
                        try {
                            ...
                            const fileContents = await Promise.all(msg.attachments.map(async (att) => {
                                try {
            ...
            const responsePreview = fullResponse.length > 100
                ...
                : fullResponse;
            sendNotification('AI Response Complete', responsePreview, {
