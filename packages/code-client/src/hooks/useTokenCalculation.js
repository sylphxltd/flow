...
function getMessageCacheKey(sessionId, messageIndex, content) {
    // Use hash of content + session + index for cache key
    return `${sessionId}:${messageIndex}:${content.substring(0, 100)}`;
}
export function useTokenCalculation(currentSession) {
...
//# sourceMappingURL=useTokenCalculation.js.map