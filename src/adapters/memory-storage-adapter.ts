...
export class MemoryStorageAdapter implements MemoryStorageAdapter {
   ...
   */
  async setBatch(entries: Array<{ key: string; value: unknown; namespace?: string }>): Promise<void> {
    await StorageUtils.executeOperation(
      async () => {
        this.ensureInitialized();
   ...
   */
  async getBatch(keys: string[], namespace = 'default'): Promise<Array<{ key: string; value: unknown }>> {
    return StorageUtils.executeOperation(
      async () => {
        this.ensureInitialized();
