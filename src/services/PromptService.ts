...
const makePromptService = Effect.gen(function* () {
  const confirm = (message: string, defaultYes = false): Effect.Effect<never, Error, boolean> =>
    Effect.tryPromise({
      try: () =>
  ...
  const input = (message: string, defaultValue = ''): Effect.Effect<never, Error, string> =>
    Effect.tryPromise({
      try: () =>
