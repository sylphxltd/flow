...
import { all, failure, map, success } from '../../core/functional/result.js';
 ...
 */
export const validateNotEmpty: FileValidator = (content: string): Result<string, FileSystemError> => {
  if (content.trim().length === 0) {
    return failure(
      fileSystemError('File content is empty', '', 'read')
    );
  }
...
export const validateSize =
  ...
  (content: string): Result<string, FileSystemError> => {
    ...
    if (bytes > maxBytes) {
      return failure(
        fileSystemError(
          `File content exceeds maximum size: ${bytes} > ${maxBytes}`,
          '',
          'read',
...
export const processFiles = (
  ...
  const results = files.map((file) =>
    processFileContent(file, transform, validator)
  );
