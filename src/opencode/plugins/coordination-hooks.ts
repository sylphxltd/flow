...
export const CoordinationHooksPlugin: Plugin = async ({ project, client, $, directory, worktree }) => {
  return {
    "event": (input) => {
      input.event 
    },
...
async function broadcastAgentEvent(agentId: string, message: any): Promise<void> {
  try {
    const eventData = {
      message: message,
      timestamp: Date.now()
