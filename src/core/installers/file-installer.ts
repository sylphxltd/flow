...
export class FileInstaller {
  ...
  async installToDirectory(
    ...
    const files = await this.collectSourceFiles(sourceDir, options.extension || '.md');
   ...
   */
  private async collectSourceFiles(sourceDir: string, extension: string): Promise<string[]> {
