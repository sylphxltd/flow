...
export class DomainError extends Error.TaggedError('DomainError')<{
  readonly message: string;
  readonly field?: string;
}>() {}
export class AppError extends Error.TaggedError('AppError')<{
  readonly message: string;
...
// Matcher for errors
export const matchError = (error: ErrorUnion) =>
  pipe(error, ({ _tag, message, code, field }) => {
    switch (_tag) {
      case 'InfraError':
