...
export const SimpleMemoryTUI: React.FC = () => {
  const { exit } = useApp();
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      namespaces.forEach(ns => {
        namespaceCounts[ns] = Object.keys((data.namespaces as any)?.[ns] || {}).length;
      });
    ...
    return (
      <Box flexDirection="column">
        ...
        <Box marginBottom={1}>
          ...
          {Object.entries(stats.namespaceCounts).map(([ns, count]) => (
            <Text key={ns}>  • {ns}: {count} entries</Text>
          ))}
        </Box>
