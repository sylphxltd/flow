...
function getDescriptionForFile(filePath?: string): string {
  if (!filePath) return 'Development workflow agent';
...
function displayResults(results: Array<{file: string, status: string, action: string}>, agentsDir: string, agentName: string): void {
...
export async function installAgents(options: { agent?: string; verbose?: boolean; dryRun?: boolean; clear?: boolean; merge?: boolean }): Promise<void> {
  const cwd = process.cwd();
  ...
  if (options.clear && fs.existsSync(agentsDir)) {
    ...
    const existingFiles = fs.readdirSync(agentsDir, { recursive: true })
      ...
      .map((file) => path.join(agentsDir, file as string));
  ...
  if (options.merge) {
    ...
    for (const batch of batches) {
      const promises = batch.map(filePath =>
        processFile(filePath, agentsDir, config.extension, processContent, config.flatten, results, progressBar)
      );
